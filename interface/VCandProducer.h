...
class VCandProducer {
  public:
    ...
    void getCands (std::vector<vlq::ResolvedVjj>&) ; 
    ~VCandProducer () ; 
  private:
    void makeCands() ; 
...
VCandProducer::VCandProducer (const vlq::JetCollection& jets, 
    const double massMin, const double massMax, 
    const double drjjMax,const double scaledmassdropMin, const double scaledmassdropMax) :
  jets_(jets),
  massMin_(massMin),
...
VCandProducer::~VCandProducer () {} 

void VCandProducer::makeCands () {
...
void VCandProducer::getCands (std::vector<vlq::ResolvedVjj>& cands) { 
