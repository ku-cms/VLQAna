...
class TprimeMVA : public edm::EDFilter {
  public:
    explicit TprimeMVA(const edm::ParameterSet&);
    ~TprimeMVA();
...
//
TprimeMVA::TprimeMVA(const edm::ParameterSet& iConfig) :
  ...
  scaledmassdropmax_      (iConfig.getParameter<double>            ("scaledmassdropmax"))
{
  ...
  produces<double>("bdtgwt0") ; 
  produces<double>("bdtgerr0") ; 
  produces<double>("bdtgwt1") ; 
  produces<double>("bdtgerr1") ; 
...
// ------------ method called to produce the data  ------------
  bool
TprimeMVA::filter(edm::Event& evt, const edm::EventSetup& iSetup)
{
  using namespace edm;
  ...
  Handle<double> h_htak4jets          ; evt.getByLabel (l_htak4jets , h_htak4jets) ;
  Handle<double> h_htak8jets          ; evt.getByLabel (l_htak8jets , h_htak8jets) ;
  Handle<double> h_maxetaak4          ; evt.getByLabel (l_maxetaak4 , h_maxetaak4) ;
  Handle<double> h_MassLeading2AK8    ; evt.getByLabel (l_MassLeading2AK8 , h_MassLeading2AK8) ;
  Handle<double> h_DeltaEtaLeading2AK8; evt.getByLabel (l_DeltaEtaLeading2AK8 , h_DeltaEtaLeading2AK8) ;
  Handle<double> h_pt1stAK8           ; evt.getByLabel (l_pt1stAK8 , h_pt1stAK8) ;
  Handle<double> h_pt2ndAK8           ; evt.getByLabel (l_pt2ndAK8 , h_pt2ndAK8) ;
  Handle<double> h_mass1stAK8         ; evt.getByLabel (l_mass1stAK8 , h_mass1stAK8) ;
  Handle<double> h_mass2ndAK8         ; evt.getByLabel (l_mass2ndAK8 , h_mass2ndAK8) ;
  ...
  float ngoodAK4Jets = *h_ngoodAK4Jets.product() ;
  float ngoodAK8Jets = *h_ngoodAK8Jets.product() ;
  float nbjets = *h_ngoodBTaggedAK4Jets.product() ; 
  float ntjets = *h_nTJets.product() ; 
  float nhjets = *h_nHJets.product() ; 
  float nwjets = *h_nWJets.product() ; 

  float htak4 = *h_htak4jets.product() ; 
  float htak8 = *h_htak8jets.product() ; 
  //float maxetaak4 = *h_maxetaak4.product() ; 
  //float MassLeading2AK8 = *h_MassLeading2AK8.product() ; 
  //float DeltaEtaLeading2AK8 = *h_DeltaEtaLeading2AK8.product() ;
  //float pt1stAK8 = *h_pt1stAK8.product() ;
  //float pt2ndAK8 = *h_pt2ndAK8.product() ;
  //float mass1stAK8 = *h_mass1stAK8.product() ;
  //float mass2ndAK8 = *h_mass2ndAK8.product() ;
  ...
  if ( int(nbjets) > 0 ) { 
    ...
    else if ( int(ntjets) == 0 && int(nhjets) > 0 && int(nwjets) > 0 ) { //// --> type 2
      evttype = 2 ; 
    ...
    else if ( int(ntjets) == 0 && int(nwjets) == 0 && int(nhjets) > 0 ) { //// --> type 3 
      evttype = 3 ; 
      pthjet = h_jetAK8Pt.product()->at( (h_hjetIdxs.product()->at(0)) ) ; 
      mhjet = h_jetAK8Mass.product()->at( (h_hjetIdxs.product()->at(0)) ) ; 
    }
    else if ( int(ntjets) == 0 && int(nhjets) == 0 && int(nwjets) > 0 && int(nbjets) > 1 ) { //// --> type 4
      evttype = 4 ; 
      ptwjet = h_jetAK8Pt.product()->at( (h_wjetIdxs.product()->at(0)) ) ; 
      mwjet = h_jetAK8Mass.product()->at( (h_wjetIdxs.product()->at(0)) ) ; 
    }
  ...
  TMVA::Reader *reader0 = new TMVA::Reader("Silent") ; 
  reader0->AddVariable("double_ana_htak4jets_SingleTpAna.obj",         &htak4);
  reader0->AddVariable("double_ana_htak8jets_SingleTpAna.obj",         &htak8);
  reader0->AddVariable("uint_ana_ngoodAK4Jets_SingleTpAna.obj",        &ngoodAK4Jets);
  reader0->AddVariable("uint_ana_ngoodAK8Jets_SingleTpAna.obj",        &ngoodAK8Jets);
  reader0->AddVariable("uint_ana_ngoodBTaggedAK4Jets_SingleTpAna.obj", &nbjets);

  reader0->AddVariable("double_sel_ptbleading_SingleTpAna.obj",  &ptbleading);
  reader0->AddVariable("double_sel_etableading_SingleTpAna.obj", &etableading);
  reader0->AddVariable("double_sel_ptjforward_SingleTpAna.obj",  &ptjforward);
  reader0->AddVariable("double_sel_etajforward_SingleTpAna.obj", &etajforward);

  reader0->AddVariable("double_sel_pttjet_SingleTpAna.obj", &pttjet);
  reader0->AddVariable("double_sel_mtjet_SingleTpAna.obj",  &mtjet );
  reader0->AddVariable("double_sel_pthjet_SingleTpAna.obj", &pthjet);
  reader0->AddVariable("double_sel_mhjet_SingleTpAna.obj",  &mhjet );
  reader0->AddVariable("double_sel_ptth_SingleTpAna.obj",   &ptth  );
  reader0->AddVariable("double_sel_mth_SingleTpAna.obj",    &mth   );
  reader0->AddVariable("double_sel_detath_SingleTpAna.obj", &detath);

  reader0->BookMVA("BDTG", "SingleTprimevsBkg_Global_WthWt_3_SingleTprimevsBkg_Global_WthWt_3BDTG0.weights.xml") ;
  Double_t bdtgwt0 = reader0->EvaluateMVA( "BDTG" );
  Double_t bdtgerr0 = reader0->GetMVAError();

  TMVA::Reader *reader1 = new TMVA::Reader("Silent") ; 
  reader1->AddVariable("double_ana_htak4jets_SingleTpAna.obj",         &htak4);
  reader1->AddVariable("double_ana_htak8jets_SingleTpAna.obj",         &htak8);
  reader1->AddVariable("uint_ana_ngoodAK4Jets_SingleTpAna.obj",        &ngoodAK4Jets);
  reader1->AddVariable("uint_ana_ngoodAK8Jets_SingleTpAna.obj",        &ngoodAK8Jets);
  reader1->AddVariable("uint_ana_ngoodBTaggedAK4Jets_SingleTpAna.obj", &nbjets);

  reader1->AddVariable("double_sel_ptbleading_SingleTpAna.obj",  &ptbleading);
  reader1->AddVariable("double_sel_etableading_SingleTpAna.obj", &etableading);
  reader1->AddVariable("double_sel_ptjforward_SingleTpAna.obj",  &ptjforward);
  reader1->AddVariable("double_sel_etajforward_SingleTpAna.obj", &etajforward);

  reader1->AddVariable("double_sel_pthjet_SingleTpAna.obj", &pthjet);
  reader1->AddVariable("double_sel_mhjet_SingleTpAna.obj",  &mhjet );
  reader1->AddVariable("double_sel_ptwjet_SingleTpAna.obj", &ptwjet);
  reader1->AddVariable("double_sel_mwhb_SingleTpAna.obj",  &mwhb );
  reader1->AddVariable("double_sel_drwb_SingleTpAna.obj",  &drwb );

  reader1->BookMVA("BDTG", "SingleTprimevsBkg_Global_WthWt_3_SingleTprimevsBkg_Global_WthWt_3BDTG1.weights.xml") ;
  Double_t bdtgwt1 = reader1->EvaluateMVA( "BDTG" );
  Double_t bdtgerr1 = reader1->GetMVAError();
  ...
  auto_ptr<double> bdtgwt0ptr( new double(bdtgwt0) ) ;
  evt.put(bdtgwt0ptr, "bdtgwt0") ; 
  auto_ptr<double> bdtgerr0ptr( new double(bdtgerr0) ) ;
  evt.put(bdtgerr0ptr, "bdtgerr0") ; 
  auto_ptr<double> bdtgwt1ptr( new double(bdtgwt1) ) ;
  evt.put(bdtgwt1ptr, "bdtgwt1") ; 
  auto_ptr<double> bdtgerr1ptr( new double(bdtgerr1) ) ;
  evt.put(bdtgerr1ptr, "bdtgerr1") ; 
...
/*
   void
   TprimeMVA::beginRun(edm::Run const&, edm::EventSetup const&)
   {
   }
   */
...
/*
   void
   TprimeMVA::endRun(edm::Run const&, edm::EventSetup const&)
   {
   }
   */
...
/*
   void
   TprimeMVA::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
   {
   }
   */
...
/*
   void
   TprimeMVA::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
   {
   }
   */
...
void
TprimeMVA::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
